...
type ModalContent =
    ...
    | { type: "users"; data: UserActivityRecord[] }
    | { type: "alert"; data: SecurityAlert }
    | { type: "alerts"; data: SecurityAlert[] }
    | { type: "performance"; data: PerformanceData }
    | { type: "securityMetrics"; data: SecurityMetric[] }
    | { type: "networkTraffic"; data: NetworkTrafficData[] }
    | { type: "storage"; data: StorageData[] };
...
const CyberpunkDashboard = () => {
    ...
    const userActivityData: UserActivityRecord[] = [
    ...
    const networkTrafficData: NetworkTrafficData[] = [
    ...
    const securityAlerts: SecurityAlert[] = [
        {
            level: "critical",
            message: "Critical Breach Attempt",
            time: "2m ago",
            details:
                "Attempted intrusion detected from IP 192.168.1.42. Firewall blocked 3 unauthorized access attempts to the primary database.",
        },
        {
            level: "warning",
            message: "Unusual Login Pattern",
            time: "17m ago",
            details:
                "User admin_johnson logged in from an unrecognized location. Geographical anomaly detected.",
        },
        {
            level: "warning",
            ...
            time: "25m ago",
            details:
                "User account marketing_lead has had 5 failed login attempts in the past 10 minutes. Account temporarily locked.",
        },
        {
            level: "info",
            ...
            time: "1h ago",
            details:
                "Security patch KB-29471 is available for installation. Recommended to apply during next maintenance window.",
        },
    ];
        ...
        <div className={`min-h-screen bg-gray-900 text-cyan-400 font-mono p-4 ${glitchEffect ? "opacity-90" : ""}`}>
            ...
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 border-b border-cyan-700/50 pb-4 relative overflow-hidden">
                <div className="relative">
                    <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-pink-400 mb-2">
                        NEXUS-9 // QUANTUM INTERFACE
                    </h1>
                ...
                <div className="flex flex-wrap gap-3 mt-4 lg:mt-0">
                    <div
                        onClick={() =>
                            openModal({ type: "system", data: { cpu: animatedValues.cpu, memory: animatedValues.memory, network: animatedValues.network } })
                        }
                        className="group bg-cyan-900/30 backdrop-blur-sm p-3 rounded-md border border-cyan-700/50 hover:border-cyan-500 cursor-pointer transition-all duration-300 relative overflow-hidden"
                    >
                    ...
                    <div
                        onClick={() => openModal({ type: "memory", data: { memory: animatedValues.memory } })}
                        className="group bg-cyan-900/30 backdrop-blur-sm p-3 rounded-md border border-cyan-700/50 hover:border-cyan-500 cursor-pointer transition-all duration-300 relative overflow-hidden"
                    >
                        <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/0 via-cyan-500/10 to-cyan-500/0 opacity-0 group-hover:opacity-100 transform -translate-x-full group-hover:translate-x-0 transition-all duration-700"></div>
                    ...
                    <div
                        onClick={() => openModal({ type: "network", data: { network: animatedValues.network } })}
                        className="group bg-cyan-900/30 backdrop-blur-sm p-3 rounded-md border border-cyan-700/50 hover:border-cyan-500 cursor-pointer transition-all duration-300 relative overflow-hidden"
                    >
                        <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/0 via-cyan-500/10 to-cyan-500/0 opacity-0 group-hover:opacity-100 transform -translate-x-full group-hover:translate-x-0 transition-all duration-700"></div>
                    ...
                    >
                        ...
                        <div className="flex items-center">
                            ...
                            {notificationCount > 0 && <span className="ml-2 h-2 w-2 bg-pink-500 rounded-full animate-ping absolute"></span>}
                        </div>
                    </div>
                </div>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
                <div
                    className={`col-span-1 lg:col-span-2 border border-cyan-800/50 bg-gradient-to-br from-gray-900 to-gray-900/90 backdrop-blur-sm p-4 rounded-md relative ${
                        hoverCard === "status" ? "border-cyan-500 shadow-lg shadow-cyan-900/20" : "hover:border-cyan-700"
                ...
                <div
                    className={`border border-cyan-800/50 bg-gradient-to-br from-gray-900 to-gray-900/90 backdrop-blur-sm p-4 rounded-md relative ${
                        hoverCard === "users" ? "border-cyan-500 shadow-lg shadow-cyan-900/20" : "hover:border-cyan-700"
                    } transition-all duration-300`}
                ...
                <div
                    className={`border border-cyan-800/50 bg-gradient-to-br from-gray-900 to-gray-900/90 backdrop-blur-sm p-4 rounded-md relative ${
                        hoverCard === "security" ? "border-cyan-500 shadow-lg shadow-cyan-900/20" : "hover:border-cyan-700"
                    } transition-all duration-300`}
                ...
                >
                  ...
                  {alert.time}
                    ...
                    <button
                        className="w-full mt-3 border border-cyan-800/50 hover:border-cyan-600 bg-cyan-900/10 hover:bg-cyan-900/30 p-1 text-sm transition-all duration-200"
                        onClick={() => openModal({ type: "alerts", data: securityAlerts })}
                    >
                        VIEW ALL ALERTS
                    </button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
                <div
                    className={`lg:col-span-2 border border-cyan-800/50 bg-gradient-to-br from-gray-900 to-gray-900/90 backdrop-blur-sm p-4 rounded-md relative ${
                        hoverCard === "performance" ? "border-cyan-500 shadow-lg shadow-cyan-900/20" : "hover:border-cyan-700"
                    } transition-all duration-300`}
                    onMouseEnter={() => setHoverCard("performance")}
                    onMouseLeave={() => setHoverCard(null)}
                    onClick={() => openModal({ type: "performance", data: { records: performanceData } })}
                >
                ...
                <div
                    className={`border border-cyan-800/50 bg-gradient-to-br from-gray-900 to-gray-900/90 backdrop-blur-sm p-4 rounded-md relative ${
                        hoverCard === "radar" ? "border-cyan-500 shadow-lg shadow-cyan-900/20" : "hover:border-cyan-700"
                    } transition-all duration-300`}
                ...
                >
                    ...
                    <div className="flex justify-center items-center text-sm mt-2">
                        <span className="px-2 py-1 bg-pink-900/20 text-pink-400 border border-pink-800/50 rounded">Security Score: 92/100</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                <div
                    className={`col-span-1 md:col-span-2 border border-cyan-800/50 bg-gradient-to-br from-gray-900 to-gray-900/90 backdrop-blur-sm p-4 rounded-md relative ${
                        hoverCard === "network" ? "border-cyan-500 shadow-lg shadow-cyan-900/20" : "hover:border-cyan-700"
                    } transition-all duration-300`}
                ...
                <div
                    className={`border border-cyan-800/50 bg-gradient-to-br from-gray-900 to-gray-900/90 backdrop-blur-sm p-4 rounded-md relative ${
                        hoverCard === "storage" ? "border-cyan-500 shadow-lg shadow-cyan-900/20" : "hover:border-cyan-700"
                    } transition-all duration-300`}
                    ...
                    <div className="bg-gray-800 border border-cyan-700 rounded-md p-6 relative max-w-lg w-full mx-4">
                        ...
                        {modalContent && modalContent.type === "system" && (
                            <div>
                                ...
                                <p className="mb-2">
                                    CPU Load: <span className="text-pink-400">{modalContent.data.cpu}%</span>
                                </p>
                                <p className="mb-2">
                                    Memory Usage: <span className="text-pink-400">{modalContent.data.memory}%</span>
                                </p>
                                <p className="mb-2">
                                    Network Usage: <span className="text-pink-400">{modalContent.data.network}%</span>
                                </p>
                            </div>
                        )}
                        ...
                        {modalContent && modalContent.type === "alert" && (
                            <div>
                                ...
                                <p
                                    className={`mb-2 ${
                                        modalContent.data.level === "critical"
                                            ? "text-red-400"
                                            : modalContent.data.level === "warning"
                                                ? "text-yellow-400"
                                                : "text-blue-400"
                                    }`}
                                >
                                    Level: {modalContent.data.level.toUpperCase()}
                                </p>
                                ...
                                <div className="space-y-2 max-h-64 overflow-y-auto">
                                    {modalContent.data.map((alert, index) => (
                                        <div key={index} className="p-2 border border-cyan-700 rounded-md">
                                            ...
                                            >
                                                Level: {alert.level.toUpperCase()} - {alert.time}
                                            </p>
                        ...
                        {modalContent && modalContent.type === "performance" && (
                            <div>
                                ...
                                <ResponsiveContainer width="100%" height={300}>
                                    <AreaChart data={modalContent.data.records} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                                        <defs>
